...
export default {
    ...
    callback: async (message: Message, args: string[], text: string) => {
        ...
        if (!minStep) { (message.channel as TextChannel).send(language(message, 'ROLE_ID_NOT')); console.log("role error0"); return }
        ...
        if (minStep.position >= curStep.position) {
            (message.channel as TextChannel).send(language(message, 'NO_SPACE_FOR_DOWNGRADE'));
            return
        }
