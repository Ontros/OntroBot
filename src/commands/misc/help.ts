    ...
    callback: async (message: Message, args: string[], text: string) => {
        ...
        if (!args[0]) {
            ...
            for (const categoryName in global.commands) {
                ...
                embed.addField(category.name, global.lang(message.guild.id, `${categoryName.toUpperCase()}_DES`))
            }
            ...
            for (const commandName in category.commands) {
                if (/*commandName.toLocaleLowerCase() === args[0].toLocaleLowerCase()||*/camelize(text).toLocaleLowerCase() === commandName.toLocaleLowerCase()) {
                    ...
                    embed.setColor('#0099ff')
                        ...
                        .setThumbnail(avatarURL)
                        .addField(lang(message.guild.id, 'ALIASES'), command.aliases)
                    if (command.args) { embed.addField(lang(message.guild.id, 'ARGS'), command.args) }
                    embed.addField(lang(message.guild.id, 'DESCR'), global.lang(message.guild.id, 'DES_' + commandName.toUpperCase() + '_LONG'))
                    message.channel.send(embed)
                    return
                }
            }
