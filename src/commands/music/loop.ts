...
export default {
    ...
    callback: async (message: Message, args: string[], text: string) => {
        ...
        //     //no queue
        //     (message.channel as TextChannel).send(language(message, 'LOOP_NO'));
        //     server.loop = 0;
        // }
        // else if (['1', 'queue'].some(i => i === args[0])) {
        //     //loop queue
        //     (message.channel as TextChannel).send(language(message, 'LOOP_QUEUE'));
        //     server.loop = 1;
        // }
        // else if (['2', 'random'].some(i => i === args[0])) {
        //     //loop random queue
        //     (message.channel as TextChannel).send(language(message, 'LOOP_QUEUE_RANDOM'));
        //     server.loop = 2;
        // }
        // else if (['3', 'track'].some(i => i === args[0])) {
        //     //loop song
        //     (message.channel as TextChannel).send(language(message, 'LOOP_TRACK'));
        //     server.loop = 3;
        // }
        // else {
